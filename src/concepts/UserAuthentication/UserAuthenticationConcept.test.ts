...
Deno.test("Action: updateCredential - invalid session token", async () => {
  const [db, client] = await testDb();
  const concept = new UserAuthenticationConcept(db);

  try {
    console.log("--- Test: updateCredential - invalid session token ---");
    const invalidToken = "bad_session_for_update";
    const credentialType = "SomeType";
    const newValue = "some_value";

    console.log(`Attempting to update credential with invalid session token: ${invalidToken}`);
    const result = await concept.updateCredential({
      sessionToken: invalidToken,
      credentialType,
      newCredentialValue: newValue,
...
Deno.test("Action: updateCredential - empty credential type", async () => {
  const [db, client] = await testDb();
  const concept = new UserAuthenticationConcept(db);

  try {
    console.log("--- Test: updateCredential - empty credential type ---");
    const username = createTestUsername();
    const password = "pass";
    const credentialType = ""; // Empty string
    const newValue = "some_value";
    ...
    console.log("Attempting to update credential with empty credential type.");
    const result = await concept.updateCredential({
      sessionToken,
      credentialType,
      newCredentialValue: newValue,
