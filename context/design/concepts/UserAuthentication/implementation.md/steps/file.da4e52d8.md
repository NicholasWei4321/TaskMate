...
content_id: da4e52d80ab0a361fabd3ef8e692490889555922f94ec79eb5296e9320d8c257
---
...
export default class UserAuthenticationConcept {
  ...
  async storeCredential(
    ...
    // Update user's additionalCredentials
    // MongoDB allows dot notation to update nested fields in an object
    const updateResult = await this.users.updateOne(
      { _id: session.user },
      { $set: { [`additionalCredentials.${credentialType}`]: credentialValue } },
    );

    if (updateResult.modifiedCount === 0 && updateResult.upsertedId === null) {
      // This implies the user document was not found or not modified, which shouldn't happen if session exists.
      return { error: "Failed to store credential (user not found or not modified)" };
    }
  ...
  async updateCredential(
    ...
    // Ensure the credentialType exists before updating, as per spec implied by "updateCredential" vs "storeCredential"
    const user = await this.users.findOne({ _id: session.user });
    if (!user || !user.additionalCredentials?.[credentialType]) {
    ...
    if (updateResult.modifiedCount === 0 && updateResult.upsertedId === null) {
      return { error: "Failed to update credential (user not found or not modified)" };
    }
